...
export default function BillItemsList({ items, setItems }) {
    ...
    setItems(prev =>
      prev
        .map(i => {
          ...
          // Fixed decimal precision to avoid floating point errors
          const roundedDisplay = parseFloat(newDisplay.toFixed(3)); // 0.25 steps need 3 decimals
  ...
  const updatePrice = (id: string, price: string) => {
    ...
    setItems(prev =>
      prev.map(i => (i.productId === id ? { ...i, price: clean } : i))
    );
  };
  ...
  const renderItem = ({ item }) => {
    ...
    // Determine stepper increment based on unit type
    const isWeightOrVolume = invU === "kg" || invU === "litre";
    const stepperIncrement = isWeightOrVolume ? 0.25 : 1;
      ...
      <View style={[styles.card, { zIndex: openDropdownId === item.productId ? 100 : 1 }]}>
        ...
        <View style={styles.controlsRow}>
          ...
          <View style={styles.qtyBox}>
            ...
            <View style={styles.unitContainer}>
              ...
              {canChangeUnit && (
                <TouchableOpacity
                  onPress={() =>
                    setOpenDropdownId(openDropdownId === item.productId ? null : item.productId)
                  }
                ...
                {allowedUnits.map((u, index) => (
                  <TouchableOpacity
                    ...
                    onPress={() => {
                      setItems(prev =>
                        prev.map(i =>
                          i.productId === item.productId ? { ...i, displayUnit: u } : i
                        )
                      );
                    ...
                    style={[
                      styles.dropItem,
                      index === allowedUnits.length - 1 && { borderBottomWidth: 0 },
                    ]}
                  >
          ...
          <View style={styles.stepper}>
            <TouchableOpacity
              onPress={() => updateQuantityByDelta(item.productId, -stepperIncrement)}
