 ...
 *   (Paneer)          1  240
 *
 * Line prefixes (stripped by the printer script):
 *   !H!  — shop header (bold)
 *   !B!  — emphasis (bold)
 */
...
/** Pad / trim every line to COL so the block centers evenly. */
...
/** Header / qty+amt row: left text | Qty | Amt */
...
/**
 * @param {object|null} data  Pass null for a hardcoded test receipt.
 */
export function buildReceiptText(data) {
  ...
  (data.items || []).forEach((item) => {
    ...
    nameLines.forEach((ln, i) => {
      // Put Qty + Amt on the last name line when it fits; else own row
    ...
    // Unit price hint under the item (compact)
...
/** Exported for the print script so centering matches line width. */
export const RECEIPT_COL = COL;
